...
color_list = ["#fffb77", "#fffa77", "#fff977", "#fff876", "#fff776",
              "#fff676", "#fff576", "#fff475", "#fff375", "#fff275",
              "#fff175", "#fff075", "#ffef74", "#ffef74", "#ffee74",
              "#ffed74", "#ffec74", "#ffeb73", "#ffea73", "#ffe973",
              "#ffe873", "#ffe772", "#ffe672", "#ffe572", "#ffe472",
              "#ffe372", "#ffe271", "#ffe171", "#ffe071", "#ffdf71",
              "#ffde70", "#ffdd70", "#ffdc70", "#ffdb70", "#ffda70",
              "#ffd96f", "#ffd86f", "#ffd76f", "#ffd66f", "#ffd66f",
              "#ffd56e", "#ffd46e", "#ffd36e", "#ffd26e", "#ffd16d",
              "#ffd06d", "#ffcf6d", "#ffce6d", "#ffcd6d", "#ffcc6c",
              "#ffcb6c", "#ffca6c", "#ffc96c", "#ffc86b", "#ffc76b",
              "#ffc66b", "#ffc56b", "#ffc46b", "#ffc36a", "#ffc26a",
              "#ffc16a", "#ffc06a", "#ffbf69", "#ffbe69", "#ffbd69",
              "#ffbd69", "#ffbc69", "#ffbb68", "#ffba68", "#ffb968",
              "#ffb868", "#ffb768", "#ffb667", "#ffb567", "#ffb467",
              "#ffb367", "#ffb266", "#ffb166", "#ffb066", "#ffaf66",
              "#ffad65", "#ffac65", "#ffab65", "#ffa964", "#ffa864",
              "#ffa763", "#ffa663", "#ffa463", "#ffa362", "#ffa262",
              "#ffa062", "#ff9f61", "#ff9e61", "#ff9c61", "#ff9b60",
              "#ff9a60", "#ff9860", "#ff975f", "#ff965f", "#ff955e",
              "#ff935e", "#ff925e", "#ff915d", "#ff8f5d", "#ff8e5d",
              "#ff8d5c", "#ff8b5c", "#ff8a5c", "#ff895b", "#ff875b",
              "#ff865b", "#ff855a", "#ff845a", "#ff8259", "#ff8159",
              "#ff8059", "#ff7e58", "#ff7d58", "#ff7c58", "#ff7a57",
              "#ff7957", "#ff7857", "#ff7656", "#ff7556", "#ff7455",
              "#ff7355", "#ff7155", "#ff7054", "#ff6f54", "#ff6d54",
              "#ff6c53", "#ff6b53", "#ff6953", "#ff6852", "#ff6752",
              "#ff6552", "#ff6451", "#ff6351", "#ff6250", "#ff6050",
              "#ff5f50", "#ff5e4f", "#ff5c4f", "#ff5b4f", "#ff5a4e",
              "#ff584e", "#ff574e", "#ff564d", "#ff544d", "#ff534d",
              "#ff524c", "#ff514c", "#ff4f4b", "#ff4e4b", "#ff4d4b"]
...
def monte_carlo(data, test_size=0.5, simulation=100, **kwargs):
    # train test split as usual
    train_data, test_data = train_test_split(data, test_size=test_size, shuffle=False, **kwargs)
    forecast_horizon = len(test_data)

    # we only care about close price
    # if there has been dividend issued
    # we use adjusted close price instead
    train_data = train_data.loc[:, ["Close"]]

    # here we use log return
    returnn = np.log(train_data["Close"].iloc[1:] / train_data["Close"].shift(1).iloc[1:])
    ...
    for counter in range(simulation):
        d[counter] = [train_data["Close"].iloc[0]]
        ...
        for i in range(len(train_data) + forecast_horizon - 1):
    ...
    for counter in range(simulation):
        temp = np.std(np.subtract(
            d[counter][:len(train_data)], train_data["Close"]))
        if temp < std:
            std = temp
            pick = counter
...
def test(df, ticker, simu_start=100, simu_end=1000, simu_delta=100, **kwargs):
    ...
    plt.barh(np.arange(1, len(table) * 2 + 1, 2), table["Prediction"],
             color=color_list[0::int(len(color_list) / len(table))])
